...
The `home-routes` handler in the `next-transit.routes.home` namespace
...
The `home-page` function will in turn call the `next-transit.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `next-transit.handler` namespace:
...
Request middleware functions are located under the `next-transit.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `next-transit.dev-middleware` namespace found in
